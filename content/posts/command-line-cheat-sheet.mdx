...
```bash
$ cat <<EOF | tr -d "[:space:]" | readability
<!DOCTYPE html>
<html>
    <head>
...
## List most recently added file
Output the absolute file path for the most recently added file in a directory.

```
$ ls -dt [DIRECTORY]/* | head -1
```
...
## Curl and extract tgz
```
curl -fsL [TGZ_URL] | tar xfz - -C [DESTINTION]
```
