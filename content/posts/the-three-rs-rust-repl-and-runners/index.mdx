...
These steps can grow tedious, especially when making a new cargo package. The iterations can be shortened by more work up front looking at documentation, source, and examples. But sometimes you just want to experiment in a quick and efficient way. That is when REPLs and runners can prove to be helpful.
...
I did some digging to find an official Rust REPL and there is an open issue to officially [implement a proper REPL](https://github.com/rust-lang/rfcs/issues/655).
...
A Rust runner allows you to run (well, compile, link, _and_ run) Rust code without the overhead (and benefits) of creating a Cargo package. Similar to the REPLs, there isn’t an official Rust runner, besides Cargo, and there are a few options:
